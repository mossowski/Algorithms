...
public class Maxima {
    ...
    public ArrayList<ArrayList<MaximaPoint>> detectMaxima(ArrayList<MaximaPoint> aData) {
        ...
        for (int i = 1; i < theLeftData.size(); i++) {
            ...
            /*
             * if (y < yMin) { aLeftData.get(i).setOrientation("MIN"); itsResultLeft.add(aLeftData.get(i)); yMin = y; }
             */
        }
        ...
        for (int i = theRightData.size() - 1; i > 2; i--) {
            ...
            /*
             * if (y < yMin) { theRightData.get(i).setOrientation("MIN"); itsResultRight.add(theRightData.get(i)); yMin
             * = y; }
             */
        }
    ...
    public void printResult(ArrayList<ArrayList<MaximaPoint>> aResult) {
        ...
        if (theLeft != null) {
            ...
            for (int i = 0; i < theLeft.size(); i++) {
                System.out.println("X : " + theLeft.get(i).getX() + " Y: " + theLeft.get(i).getY() + " ORIENT: "
                        + theLeft.get(i).getOrientation());
            }
            System.out.println("-------------------------------------------------------------------------");
        }
        ...
        if (theRight != null) {
            ...
            for (int i = 0; i < theRight.size(); i++) {
                System.out.println("X : " + theRight.get(i).getX() + " Y: " + theRight.get(i).getY() + " ORIENT: "
                        + theRight.get(i).getOrientation());
            }
            System.out.println("-------------------------------------------------------------------------");
        }
    }
